...
- **Implication for continuous-loss durability.** Because the raw probability stays pinned high under continuous loss, the sustained-extreme (P2) path should hold on its own once the worker no longer stalls - so a separate *level* signal is likely **unnecessary**. (If one were ever needed, the durable non-adaptive candidates are the TCP retransmit counter on the inter-node socket, `ss -tino` `bytes_retrans`, which climbs through loss and freezes on clear, or a self-computed inter-arrival jitter; not aten's private inter-arrival record.) Treat "need a level signal" as a hypothesis to confirm/refute with the post-fix retest, not a settled requirement.
